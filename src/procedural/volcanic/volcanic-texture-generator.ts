...
const rockBase:     Vec3 = { x: 0.060, y: 0.010, z: 0.008 }; // near-black dark maroon
const rockMid:      Vec3 = { x: 0.160, y: 0.032, z: 0.020 }; // slightly warmer dark red facets
// Cooled / active lava
const lavaCooled:   Vec3 = { x: 0.320, y: 0.040, z: 0.010 }; // dark red cooled crust
const lavaRed:      Vec3 = { x: 0.800, y: 0.130, z: 0.015 }; // bright glowing red
const lavaOrange:   Vec3 = { x: 0.980, y: 0.450, z: 0.040 }; // orange-hot core

// ---------------------------------------------------------------------------
// Emissive palettes (sRGB, encodes lava glow color directly)
// ---------------------------------------------------------------------------
const emitBlack:    Vec3 = { x: 0.000, y: 0.000, z: 0.000 };
const emitCooling:  Vec3 = { x: 0.380, y: 0.030, z: 0.004 }; // deep red dim glow at edges
const emitRed:      Vec3 = { x: 0.850, y: 0.100, z: 0.010 }; // bright red
const emitOrange:   Vec3 = { x: 1.000, y: 0.500, z: 0.060 }; // orange at the hottest cores
...
async function getOrCreateVolcanicMapsAsync(seed: string): Promise<VolcanicMaps> {
    ...
    const lakeThresh     = 0.660 - lavaCoverage * 0.30;  // [0.555 .. 0.624]
    ...
    const riverThresh     = 0.760 - lavaCoverage * 0.18;  // [0.726 .. 0.744]
    ...
    for (let y = 0; y < INTERNAL_HEIGHT; y++) {
        ...
        const flatMask = POLAR_DETAIL_MIN +
        ...
        for (let x = 0; x < INTERNAL_WIDTH; x++) {
            ...
            const lakeMask = flatMask * smoothstep(lakeThresh, lakeThresh + lakeTransition, lakeNorm);
            ...
            const lakeProximity = smoothstep(0.30, 0.55, lakeNorm);
            const riverMask = flatMask
            ...
            const col = mix3(
                mix3(rockCol,  lavaCooled, smoothstep(0.00, 0.20, lavaT)),
                mix3(lavaRed,  lavaOrange, smoothstep(0.55, 1.00, lavaT)),
            ...
            let ec = mix3(emitBlack,   emitCooling, smoothstep(0.00, 0.22, lavaT));
            ec     = mix3(ec,          emitRed,     smoothstep(0.18, 0.65, lavaT));
            ec     = mix3(ec,          emitOrange,  smoothstep(0.60, 1.00, lavaT));
    ...
    for (let y = 0; y < INTERNAL_HEIGHT; y++) {
        ...
        const v01Pole = y / Math.max(1, INTERNAL_HEIGHT - 1);
        const pole01 = Math.abs(v01Pole - 0.5) / 0.5;
        const normalFlatMask = POLAR_DETAIL_MIN +
            (1 - POLAR_DETAIL_MIN) * (1 - smoothstep(POLAR_FLAT_START, POLAR_FLAT_END, pole01));
