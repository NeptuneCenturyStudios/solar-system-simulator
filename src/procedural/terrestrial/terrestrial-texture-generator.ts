...
const POLAR_DETAIL_MIN = 0.30;

// Ice cap latitude
const ICE_LAT_START = 0.80;
...
const THEME_RUSTY: ColorTheme = {
    ...
    baseMid:   { x: 0.20, y: 0.10, z: 0.05 },   // rusty red-brown
...
const THEME_SANDY: ColorTheme = {
    name: 'sandy',
    baseDark:  { x: 0.13, y: 0.10, z: 0.06 },   // dark tan
...
const THEME_BASALT: ColorTheme = {
    ...
    baseLight: { x: 0.19, y: 0.19, z: 0.20 },      // medium-dark gray
    highlight: { x: 0.28, y: 0.28, z: 0.30 },      // medium gray
    accent1:   { x: 0.07, y: 0.08, z: 0.10 },      // dark bluish-gray
...
const THEME_OLIVE_TAN: ColorTheme = {
    ...
    baseLight: { x: 0.30, y: 0.25, z: 0.14 },   // light olive
    highlight: { x: 0.40, y: 0.34, z: 0.21 },   // pale tan
    accent1:   { x: 0.13, y: 0.11, z: 0.05 },   // greenish-brown
    accent2:   { x: 0.24, y: 0.20, z: 0.11 },   // golden-olive
...
function themeColor(cb: number, sub: number, theme: ColorTheme): Vec3 {
    ...
    if (t < 0.15) {
        ...
        const u = (t - 0.80 + sv) / 0.12;
        col = mix3(theme.baseLight, theme.highlight, clamp01(u));
    } else {
        // Brightest crust
        const u = (t - 0.92 + sv) / 0.08;
        col = mix3(theme.highlight, { x: 0.55, y: 0.50, z: 0.45 }, clamp01(u));
    }
...
function renderColorAndHeight(
    ...
    up: Vec3, east: Vec3, north: Vec3,
    lonCos: Float32Array, lonSin: Float32Array,
    latSin: Float32Array, latCos: Float32Array,
    ...
    for (let y = 0; y < INTERNAL_HEIGHT; y++) {
        ...
        const flatMask = POLAR_DETAIL_MIN + (1 - POLAR_DETAIL_MIN) * (1 - smoothstep(POLAR_FLAT_START, POLAR_FLAT_END, pole01));
        ...
        for (let x = 0; x < INTERNAL_WIDTH; x++) {
            ...
            const cRim = smoothstep(CRATER_RIM_EDGE0, CRATER_RIM_EDGE1, cRidged) -
                         smoothstep(CRATER_INNER_EDGE0, CRATER_INNER_EDGE1, cRidged);

            const craterH = (cRim * CRATER_RIM_STRENGTH - cInterior * CRATER_DEPTH_STRENGTH) * cGate * flatMask;
            ...
            const crackN = fbm3D(
                ...
                NOISE_CRACK_OCTAVES, seedU32
            );
            const crackRidged = Math.abs(crackN);
            const crackMask = flatMask * smoothstep(CRACK_THRESHOLD, CRACK_THRESHOLD + 0.20, crackRidged);

            // Composite height
            height[y * INTERNAL_WIDTH + x] =
                terrainN * 0.10 +
                craterH +
                crackRidged * CRACK_STRENGTH * crackMask;
            ...
            const regionN = fbm3D(
                ...
                NOISE_REGION_OCTAVES, seedU32 + 999
            );
            ...
            const subN = fbm3D(
                ...
                NOISE_SUB_COLOR_OCTAVES, seedU32 + 7777
            );
            ...
            if (cGate > 0) {
                ...
                col = mix3(col, theme.highlight, cRim * cGate * 0.10);
            }

            // === Crack coloring ===
            if (crackMask > 0) {
                col = mix3(col, theme.crackColor, crackMask * 0.30);
            }
            ...
            if (latIce > 0) {
                const iN = fbm3D(
                    ...
                    NOISE_ICE_OCTAVES, seedU32
                );
                const iGap = smoothstep(0.30, 0.70, clamp01(iN * 0.5 + 0.5));
                const iStr = latIce * iGap;
                const iBrt = 0.40 + 0.60 * smoothstep(ICE_LAT_START, ICE_LAT_END, absLat);
...
function renderNormal(height: Float32Array): Uint8ClampedArray {
    ...
    for (let y = 0; y < INTERNAL_HEIGHT; y++) {
        ...
        const v01Pole = y / Math.max(1, INTERNAL_HEIGHT - 1);
        const pole01 = Math.abs(v01Pole - 0.5) / 0.5;
        const nfm = POLAR_DETAIL_MIN + (1 - POLAR_DETAIL_MIN) * (1 - smoothstep(POLAR_FLAT_START, POLAR_FLAT_END, pole01));

        for (let x = 0; x < INTERNAL_WIDTH; x++) {
            const xL = x > 0 ? x - 1 : INTERNAL_WIDTH - 1;
            const xR = x + 1 < INTERNAL_WIDTH ? x + 1 : 0;
            const nx = -(height[y * INTERNAL_WIDTH + xR]! - height[y * INTERNAL_WIDTH + xL]!) * NORMAL_STRENGTH * nfm;
            const ny = -(height[yD * INTERNAL_WIDTH + x]! - height[yU * INTERNAL_WIDTH + x]!) * NORMAL_STRENGTH * nfm;
...
async function getOrCreateTerrestrialMapsAsync(seed: string): Promise<TerrestrialMaps> {
    const key = seed.trim();
    if (colorCache.get(key) && normalCache.get(key)) return { color: colorCache.get(key)!, normal: normalCache.get(key)! };
    ...
    const ox = rng.range(-200, 200), oy = rng.range(-200, 200), oz = rng.range(-200, 200);
    ...
    for (let x = 0; x < INTERNAL_WIDTH; x++) {
        ...
        lonCos[x] = Math.cos(lon); lonSin[x] = Math.sin(lon);
    }
    ...
    for (let y = 0; y < INTERNAL_HEIGHT; y++) {
        ...
        latSin[y] = sl; latCos[y] = Math.sqrt(Math.max(0, 1 - sl * sl));
    }

    const { data, height } = renderColorAndHeight(s32, ox, oy, oz, up, east, north, lonCos, lonSin, latSin, latCos);
    ...
    nCanvas.width = INTERNAL_WIDTH; nCanvas.height = INTERNAL_HEIGHT;
...
export function getTerrestrialTexture(seed: string): THREE.Texture { return getOrCreateTerrestrialMapsSync(seed).color; }
export function getTerrestrialNormalTexture(seed: string): THREE.Texture { return getOrCreateTerrestrialMapsSync(seed).normal; }
export function getTerrestrialTextureAsync(seed: string): Promise<THREE.Texture> { return getOrCreateTerrestrialMapsAsync(seed).then(m => m.color); }
export function getTerrestrialNormalTextureAsync(seed: string): Promise<THREE.Texture> { return getOrCreateTerrestrialMapsAsync(seed).then(m => m.normal); }
