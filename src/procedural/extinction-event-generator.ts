...
import { ExtinctionEventScenario } from '../scenarios/extinction-event-scenario';
...
import {
    EARTH_MASS,
    EXTINCTION_EVENT_SHIP_ALTITUDE,
    EXTINCTION_EVENT_TIME_SCALE,
...
/**
 * Scenario: the Sun, Earth on its real orbit, and the Moon — plus the player's Osiris
 * Mothership parked just off Earth's day side. The generator hands that ship back as a
 * launch option so the player starts in flight mode, and returns an
 * {@link ExtinctionEventScenario} that the scenario manager runs to throw the whole
 * extinction-level onslaught at Earth from the start.
 */
export class ExtinctionEventGenerator extends SolarSystemGenerator {
    ...
    constructor(dependencies: IStateDependencies, scene: THREE.Scene, seed?: string) {
        ...
        // Seeds the skydome and the threat field; the body layout itself is fixed.
        this.masterSeed = inputSeed.length > 0 ? inputSeed : generateSeedString();
        this.seed = this.masterSeed;
        console.info('[extinction-event] using master seed:', this.masterSeed);
    }

    /**
     * Build the player's Osiris Mothership on the sunward side of Earth, so Earth is lit when
     * the player looks back at it, holding a circular orbit around Earth so it doesn't fall
     * while the player gets oriented.
     */
    private createPlayerShip(sun: Sun, earth: Earth): Spaceship {
        ...
        const position = earth.mesh.position
            .clone()
            .addScaledVector(sunward, EXTINCTION_EVENT_SHIP_ALTITUDE);
        ...
        const ship = getShipTypeById('osiris_mothership').create(
            this.dependencies,
            this.scene,
            position,
            velocity,
    ...
    async generateSolarSystemAsync(
        ...
        return {
            ...
            options: {
                timeScale: EXTINCTION_EVENT_TIME_SCALE,
                playerShip: ship,
            },
            scenario: new ExtinctionEventScenario(
                this.dependencies,
                this.scene,
                earth,
                ship,
