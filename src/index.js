...
function generateIAUName(type, parentBody = null) {
    ...
    function moonName(parent) {
        ...
        const existing = bodies.filter((b) => b.name && b.name.startsWith(parent.name + ' ')).length
...
function createStatsTexture(body, bodiesArray = []) {
    ...
    if (body.temperature) {
        drawStat('Temperature: ', formatNumber(body.temperature, { minimumFractionDigits: 0, maximumFractionDigits: 0 }) + 'K', y)
        y += lineHeight
    }
...
const camera = new THREE.PerspectiveCamera(60, window.innerWidth / window.innerHeight, 1, CAMERA_FAR_PLANE)
...
function moveSelectedBodyRelativeToCamera(directionKey, ctrlKey = false) {
    ...
    if (gizmo.target === body) {
        ...
        if ((isChangingVelocity || isMiddleMouseVelocity) && velocityTipIndicator && velocityTipRing) {
            const speed = body.velocity.length()
            const arrowScale = 50
            const direction =
                speed > 0 ? body.velocity.clone().normalize() : new THREE.Vector3(1, 0, 0)
            const arrowTip = body.mesh.position.clone().add(direction.multiplyScalar(speed * arrowScale))
...
for (let i = 0; i < kuiperBeltCount; i++) {
    ...
    const r = KUIPER_BELT_INNER_DIST + Math.random() * (KUIPER_BELT_OUTER_DIST - KUIPER_BELT_INNER_DIST)
...
function getRandomStarSpawnPosition() {
    const hasAnyExistingStar = bodies.some((b) => b && !b._isDisposed && isBodyType(b, BodyType.Star))
...
function animate() {
    ...
    if (sunBody) {
        for (const body of bodies) {
            if (body && !body._isDisposed && body.mesh && !isBodyType(body, BodyType.Star) && body.baseColor) {
    ...
    if (
        ...
        if (yAxisIndicator && yAxisRing && (isChangingVelocity || isMiddleMouseVelocity || isRepositioning)) {
            updatePositionIndicator(yAxisIndicator, yAxisRing, gizmo.target.mesh.position)
        }
...
mainPanel.on('timeScaleChange', ({ value }) => {
    ...
    if (isPaused) {
        ...
        mainPanel.updateTimeScaleDisplay('0.0x (PAUSED - next: ' + absSpeed + 'x' + direction + ')')
    } else {
    ...
    ({
        ...
        try {
            const isLowMassAsteroid =
                isBodyType(body, BodyType.Asteroid) && typeof body.mass === 'number' && body.mass < 1
