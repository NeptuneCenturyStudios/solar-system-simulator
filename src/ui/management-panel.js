    ...
    initialize() {
        ...
        const updateAddModeVisibility = () => {
            ...
            if (bodyType && bodyType !== 'sun') {
                ...
                syncCreateDisplay(this.createTemperatureSlider, this.createTemperatureDisplay, (value) =>
                    formatNumberForDisplay(value) + 'K',
                )
                syncCreateDisplay(
        ...
        if (this.createLightIntensitySlider && this.createLightIntensityDisplay) {
            this.createLightIntensitySlider.oninput = () => {
                ...
                this.createLightIntensityDisplay.textContent = formatLightIntensityForDisplay(value)
            }
            this.createLightIntensitySlider.oninput()
        }
        ...
        if (this.editLightIntensitySlider) {
            this.editLightIntensitySlider.oninput = () => {
                ...
                if (this.editLightIntensityDisplay) {
                    this.editLightIntensityDisplay.textContent = formatLightIntensityForDisplay(value)
                }
            }
            this.editLightIntensitySlider.oninput()
        }
        ...
        if (this.applyEditBtn) {
            this.applyEditBtn.onclick = () => {
                ...
                const temperature = this.editTempSlider ? parseFloat(this.editTempSlider.value) : null
                const lightIntensity = this.editLightIntensitySlider
                    ? parseFloat(this.editLightIntensitySlider.value)
                    : null
                const radius = this.editRadiusSlider ? parseFloat(this.editRadiusSlider.value) : null
    ...
    setSelectedBody(body) {
        ...
        if (this.editColorInput) {
            ...
            if (this.editColorInput.parentElement) {
                this.editColorInput.parentElement.style.display = showColorPicker ? 'block' : 'none'
            }
        ...
        if (this.editRadiusSlider && this.editRadiusDisplay) {
            ...
            if (typeof this.editRadiusSlider.oninput === 'function') this.editRadiusSlider.oninput()
        }
        ...
        if (isStarBody && this.editLightIntensitySlider && this.editLightIntensityDisplay) {
            ...
            safeSetText(this.editLightIntensityDisplay, this.formatLightIntensityForDisplay(intensity))
        }

        if (this.editMassInput && this.editMassDisplay) {
            const mass =
                typeof body.mass === 'number' && isFinite(body.mass) ? body.mass : 0.001
        ...
        if (this.editVelocitySlider && this.editVelocityDisplay) {
            const velocity = typeof body.velocity?.length === 'function' ? body.velocity.length() : 0
